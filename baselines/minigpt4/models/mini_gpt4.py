...
class MiniGPT4(Blip2Base):
    ...
    def generate(
        ...
        self.llama_model.config.img_start_pos = img_start_pos + 1
        self.llama_model.config.img_length = 32
        ...
        if isinstance(outputs, dict):
            return outputs, key_position
        else:
            if len(outputs) == 3:
                output_ids, hallucination_influences, non_hallucination_influences = outputs
                ...
                output_text = self.llama_tokenizer.batch_decode(output_ids, skip_special_tokens=True)
                output_text = [text.split('###')[0].split('Assistant:')[-1].strip() for text in output_text]

                return output_text, hallucination_influences, non_hallucination_influences
            else:
    ...
    def from_config(cls, cfg):
        ...
        if ckpt_path:
            ...
            finetune_ckpt_path = cfg.get("merged_ckpt", None)
            import pdb; pdb.set_trace()
            if finetune_ckpt_path is not None and finetune_ckpt_path != 'None':
                print("Load Fine-tuned Checkpoint: {}".format(finetune_ckpt_path))
                finetune_ckpt = torch.load(finetune_ckpt_path, map_location="cpu")
                finetune_ckpt = {k: v.to(torch.float32) for k, v in finetune_ckpt['model'].items()} # # Convert weights to float32
                model.load_state_dict(finetune_ckpt, strict=False)
